...
import numpy as np
np.set_printoptions(precision=1, floatmode='fixed') # Создаю условия для будущего вещественного массива, сколько знаков после запятой
...
def main():
    ...
    while True:
        ...
        if key == '1':  # если команда 1, то заполняем массив заново
            ...
            if (random_float_array < 1 ).all(): # Проверяю наличие 0 в Массиве, так как значения float, то чистый 0 получить очень сложно
                                                # Поэтому воспользуемся хитростью и укажем, что если в Массиве число меньше 1,то добавляем в счечтик 1
                one_count += 1                  # Пользуемся all,который в nuppy служет булевым значением и определяет истина или ложь
            ...
            if one_count == 0:
                ...
                random_float_array = np.where(random_float_array <= 0, 0, 1)  # Пользуемся np.where, который  меняет вещ значения на 1
